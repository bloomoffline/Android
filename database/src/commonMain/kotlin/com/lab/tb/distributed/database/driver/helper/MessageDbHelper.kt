package com.lab.tb.distributed.database.driver.helper

import com.lab.tb.distributed.database.driver.BaseDatabase
import com.lab.tb.distributed.database.models.MessageDb
import com.lab.tb.distributed.database.models.MessageQueries

class MessageDbHelper: BaseDatabase<MessageDb, MessageQueries>() {
    override fun query(): MessageQueries {
        return db.messageQueries
    }

    override fun get(id: String): MessageDb? {
        return selectOne { selectById(id) }
    }

    override fun set(obj: MessageDb): Boolean {
        return doQuery { insert(obj) }
    }

    override fun delete(id: String): Boolean {
        return doQuery { removeById(id) }
    }

    override fun deleteAll(): Boolean {
        return doQuery { removeAll() }
    }

    override fun getAll(): List<MessageDb> {
        return selectList { selectAll() }
    }
}